 ...
 */
{
  ...
  console.log(Intl.DateTimeFormat('ko', { 
    dateStyle: 'long' 
  }).format(date)); // 2023년 11월 26일

  // timeStyle을 지정해서 시간까지 출력할 수 있다.
  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'long',
    timeStyle: 'full'
  }).format(date)); // 2023년 11월 26일 오후 7시 40분 30초 대한민국 표준시

  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'medium',
    timeStyle: 'medium'
  }).format(date)); // 2023. 11. 26. 오후 7:40:30

  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'short',
    timeStyle: 'short'
  }).format(date)); // 23. 11. 26. 오후 7:40
  ...
  console.log(Intl.DateTimeFormat('ko', {
    ...
    timeStyle: 'medium',
    hourCycle: 'h24'
  }).format(date)); // 2023. 11. 26. 19:40:30
  ...
  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'medium'
  }).formatRange(startDate, endDate)); // 2023. 3. 8. ~ 2024. 3. 31.

  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'medium',
    timeStyle: 'long',
    hourCycle: 'h24'
  }).formatRange(startDate, endDate)); // 2023. 3. 8. 19:00:00 ~ 2024. 3. 31. 21:00:00

  console.log(Intl.DateTimeFormat('ko', {
    dateStyle: 'medium',
    timeStyle: 'long'
  }).formatRange(startDate, endDate)); // 2023. 3. 8. 오후 7시 0분 0초 GMT+9 ~ 2024. 3. 31. 오후 9시 0분 0초 GMT+9
  ...
  const startDateUTC = new Date(startDate.getTime() + offsetMinutes * 60 * 1000);
 ...
 */
{
   ...
   */
  console.log(Intl.NumberFormat('ko', { 
    style: 'currency', 
    currency: 'KRW' 
  ...
  console.log(Intl.NumberFormat('ko', { 
    style: 'percent' 
  }).format(0.99)); // 99%, 기본 signDisplay는 auto이며 음수 부호만 표시한다.

  console.log(Intl.NumberFormat('ko', { 
    style: 'percent', 
    signDisplay: 'always' 
  }).format(0.8)); // +80%, always는 부호를 표시한다.
  ...
  console.log(Intl.NumberFormat('ko', { 
    style: 'percent', 
    signDisplay: 'never' 
  }).format(-0.4)); // 40%, never은 부호를 표시하지 않는다.
}